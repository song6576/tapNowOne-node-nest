-- 多供应商 AI 模型目录 + 可持久化生成任务
-- 在现有数据库执行一次；脚本可重复执行模型种子部分。

ALTER TABLE `ai_model`
  ADD COLUMN `provider` VARCHAR(32) NOT NULL DEFAULT 'dashscope' AFTER `slug`,
  ADD COLUMN `provider_model_id` VARCHAR(128) NULL AFTER `provider`;
...
ALTER TABLE `ai_model`
  MODIFY COLUMN `provider_model_id` VARCHAR(128) NOT NULL;
